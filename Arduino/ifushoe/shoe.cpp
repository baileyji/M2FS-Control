...
void ShoeDrive::init() {
  ...
  _zero_offset.pipe=0;
  _zero_offset.height=0;
  _cfg.pipe_tol = DEFAULT_TOL;
  _cfg.height_tol = DEFAULT_TOL;
  _cfg.desired_slit = UNKNOWN_SLIT;
  const uint16_t default_heights[N_SLIT_POS] = {3808, 3636, 2506, 2457, 2408, 1229};
  for (uint8_t i=0;i<N_SLIT_POS;i++) {
    _cfg.pipe_pos[i]=PIPE_SPACING*i+184;
    _cfg.height_pos[i] = default_heights[i];
    _cfg.down_pos[i] = 1229;
  }
  _cfg.down_pos[N_SLIT_POS-1]=246;
  keepSafe=true;
}
...
uint8_t ShoeDrive::getCurrentSlit(){  //0-5 or 0xFF = INTERMEDIATE/UNKNOWN, 0xFE = MOVING (really move in progress, might not be physically moving)
  uint8_t ndx;
...
void ShoeDrive::calibrateFeedbackScale() {
  int fb_min, fb_max, range;

  _moveInProgress=SHOE_CALIBRATING;

  // Height Calibration
  fb_min=getRAMSetting16(_height_motor, JrkFeedbackMinimum);
  fb_max=getRAMSetting16(_height_motor, JrkFeedbackMaximum);
  range=fb_max-fb_min;

  setRAMSetting16(_height_motor, JrkFeedbackErrorMinimum, 0);
  setRAMSetting16(_height_motor, JrkFeedbackErrorMaximum, 4095);
  setRAMSetting16(_height_motor, JrkFeedbackMinimum, 0);
  _moveHeight(0);
  delay(3000);
  _height_motor->stopMotor();
  delay(30);
  _zero_offset.height=_height_motor->getFeedback();
  setRAMSetting16(_height_motor, JrkFeedbackMinimum, fb_min);
//  setRAMSetting16(_height_motor, JrkFeedbackMinimum, _zero_offset.height);
//  setRAMSetting16(_height_motor, JrkFeedbackMaximum, _zero_offset.height+range);

  Serial.print(F("#Heightcal: \n#  Old:"));Serial.print(fb_min);
  Serial.print("-");Serial.println(fb_max);
  Serial.print(F("#  New:"));Serial.print(_zero_offset.height);
  Serial.print("-");Serial.println(_zero_offset.height+range);

  //Pipe Calibration
  fb_min=getRAMSetting16(_pipe_motor, JrkFeedbackMinimum);
  fb_max=getRAMSetting16(_pipe_motor, JrkFeedbackMaximum);
  range=fb_max-fb_min;

  setRAMSetting16(_pipe_motor, JrkFeedbackErrorMinimum, 0);
  setRAMSetting16(_pipe_motor, JrkFeedbackErrorMaximum, 4095);
  setRAMSetting16(_pipe_motor, JrkFeedbackMinimum, 0);
  _movePipe(0);
  delay(1000);
  _pipe_motor->stopMotor();
  delay(30);
  _zero_offset.pipe=_pipe_motor->getFeedback();
  setRAMSetting16(_pipe_motor, JrkFeedbackMinimum, fb_min);
//  _pipe_motor->setRAMSetting16(JrkFeedbackMinimum, _zero_offset.pipe);
//  _pipe_motor->setRAMSetting16(JrkFeedbackMaximum, _zero_offset.pipe+range);

  Serial.print(F("#Pipecal: \n#  Old:"));Serial.print(fb_min);
  Serial.print("-");Serial.println(fb_max);
  Serial.print(F("#  New:"));Serial.print(_zero_offset.pipe);
  Serial.print("-");Serial.println(_zero_offset.pipe+range);
...
void ShoeDrive::_updateFeedbackPos() { 
  ...
  pos.pipe=_pipe_motor->getScaledFeedback();
  pos.height=_height_motor->getScaledFeedback();
  _feedback_pos=pos;
...
void ShoeDrive::tellFeedbackPosition() {
  ...
  switch (_moveInProgress) {
    ...
    */
    case SLIT_MOVE: //new move, lower cassette
      ...
      Serial.print(F("#Moving to ")); Serial.print((uint16_t)_cfg.desired_slit+1);
      Serial.print(F(". Lower to height "));Serial.println(_safe_pipe_height);
      _moveHeight(_safe_pipe_height);
      _moveInProgress=SLIT_MOVE_lower;
      break;
